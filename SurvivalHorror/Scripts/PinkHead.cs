...
public partial class PinkHead : Enemy
{
    enum State{IDLE,RUN,HIT,DIE,ATTACK1,ATTACK2}
    [Export] Node3D headModel;
    [Export] GpuParticles3D headExplode;
    ...
    [Export] int maxHeadHP;
    [Export] AudioStreamPlayer3D breathing;
    [Export] SoundData swipeSFX,smackedPlayer,headDie,thud,die1,die2;
    Dictionary<State,float> dict = new Dictionary<State, float>();
   [Export] WeakPoint head;
    List<Hitbox> hitboxes = new List<Hitbox>();
    State currentState;
    bool canMove,moving,inHit,inAttack,inSound,leftSwip,randomWalkFrame,deadSlide;
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if(canMove)
        {
            ...
            if(moving)
            {
                ...
                if(!randomWalkFrame){
                    ...
                    tree.Set("parameters/RunSeek/seek_request",q);
        ...
        else
        {
            ...
            {
                YClampLookAt(Player.inst.GlobalTransform.Origin);
                if(returnToIdle <= Time.GetTicksMsec() && !dead)
                {
                    leftSwip = !leftSwip;
                    ChangeState(State.IDLE);
    ...
    public override void Die(){
        ...
        if(MiscFunctions.FiftyFifty()){
            AudioManager.inst.Play(die1,AudioType.WORLD,GlobalPosition);
        }
        else{
              AudioManager.inst.Play(die2,AudioType.WORLD,GlobalPosition);
        }
    ...
    public override void WeakPointHit(float dmg,WeakPoint weakPoint)
    {
        ...
        if(headHP > 0){
            ...
            headExplode.Emitting = true;
            headModel.Visible = false;
            AudioManager.inst.Play(headDie,AudioType.WORLD,GlobalPosition);
            Die();
        }
